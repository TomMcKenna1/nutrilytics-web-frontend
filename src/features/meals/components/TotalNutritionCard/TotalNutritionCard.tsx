...
const getTrafficLightInfo = (
  ...
  switch (nutrient) {
    case 'fats':
      if (per100g <= 3.0) { classification = 'low'; label = 'LOW'; }
      else if (per100g > 17.5) { classification = 'high'; label = 'HIGH'; }
      break;
    case 'saturatedFats':
      if (per100g <= 1.5) { classification = 'low'; label = 'LOW'; }
      else if (per100g > 5.0) { classification = 'high'; label = 'HIGH'; }
      break;
    case 'sugars':
      if (per100g <= 5.0) { classification = 'low'; label = 'LOW'; }
      else if (per100g > 22.5) { classification = 'high'; label = 'HIGH'; }
      break;
    case 'salt':
      if (per100g <= 0.3) { classification = 'low'; label = 'LOW'; }
      else if (per100g > 1.5) { classification = 'high'; label = 'HIGH'; }
      break;
    case 'protein':
      if (value < 10) { classification = 'low'; label = 'LOW'; }
      else if (value > 30) { classification = 'high'; label = 'HIGH'; }
      break;
    case 'fibre':
      if (value < 3) { classification = 'low'; label = 'LOW'; }
      else if (value > 6) { classification = 'high'; label = 'HIGH'; }
      break;
    case 'carbohydrates':
      if (value < 30) { classification = 'low'; label = 'LOW'; }
      else if (value > 70) { classification = 'high'; label = 'HIGH'; }
      break;
    default:
    ...
    <div className={styles.card}>
      ...
      <div className={styles.nutrientsGrid}>
        {nutrients.map((nutrient) => {
          ...
          const per100gValue = mealWeight > 0 ? (totalValue / mealWeight) * 100 : 0;
          const info =
            nutrient.key === 'energy'
